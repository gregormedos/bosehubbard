...
def test_decomposition_kn(term: str, num_sites: int, n_max: int):
    ...
    fig.tight_layout()
    fig.savefig(f'test/plots/decompositon_kn_{term}.pdf')
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(f'test/plots/decompositon_kn_spect_{term}.pdf')
    plt.close(fig)
...
def test_symmetries_kn(term: str, num_sites: int, n_max: int, n_tot: int):
    ...
    for energy in w:
        if energy not in spect:
            ...
            spect[energy] += 1
    spect = {energy: spect[energy] for energy in sorted(spect.keys())}
    with open(f'test/data/symmetries_kn_spect_k=zero_{term}.txt', 'w') as file:
        for energy, degeneracy in spect.items():
            file.write(f'{energy:.14f}[{degeneracy}]\n')
    ...
    for energy in w:
        if energy not in spect:
            ...
            spect[energy] += 1
    spect = {energy: spect[energy] for energy in sorted(spect.keys())}
    with open(f'test/data/symmetries_kn_spect_pk_k=zero_{term}.txt', 'w') as file:
        for energy, degeneracy in spect.items():
            file.write(f'{energy:.14f}[{degeneracy}]\n')
    ...
    for energy in w:
        if energy not in spect:
            ...
            spect[energy] += 1
    spect = {energy: spect[energy] for energy in sorted(spect.keys())}
    with open(f'test/data/symmetries_kn_spect_k=bragg_{term}.txt', 'w') as file:
        for energy, degeneracy in spect.items():
            file.write(f'{energy:.14f}[{degeneracy}]\n')
    ...
    for energy in w:
        if energy not in spect:
            ...
            spect[energy] += 1
    spect = {energy: spect[energy] for energy in sorted(spect.keys())}
    with open(f'test/data/symmetries_kn_spect_pk_k=bragg_{term}.txt', 'w') as file:
        for energy, degeneracy in spect.items():
            file.write(f'{energy:.14f}[{degeneracy}]\n')
    ...
    fig.tight_layout()
    fig.savefig(f'test/plots/symmetries_kn_{term}.pdf')
    plt.close(fig)
...
def test_decomposition_pkn(term: str, num_sites: int, n_max: int, n_tot: int):
    ...
    fig.tight_layout()
    fig.savefig(f'test/plots/decomposition_pkn_{term}.pdf')
        ...
        for sector, w_sector in w_sectors_list[i].items():
            for energy in w_sector:
                if energy not in spect:
                    ...
                    spect[energy][1] += 1
        spect = {energy: spect[energy] for energy in sorted(spect.keys())}
        with open(f'test/data/decomposition_pkn_spect_{k}_{term}.txt', 'w') as file:
            for energy, (sector, degeneracy) in spect.items():
                file.write(f'{energy:.14f}[{degeneracy}]{sector}\n')
    ...
    fig.tight_layout()
    fig.savefig(f'test/plots/decomposition_pkn_spect_{term}.pdf')
    plt.close(fig)
