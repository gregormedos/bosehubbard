...
def main1(L: int, M: int):
    ...
    dim_Z2 = []
    for Z2 in (1, -1):
        basis_Z2, dim_Z2_ = bh.gen_basis_z2block(L, Z2, M)
        dim_Z2.append(dim_Z2_)
        dim_K = []
        for K in range(L):
            representative_basis_K, trans, dim_K_ = bh.gen_representative_basis_kblock(basis_Z2, L, K)
            dim_K.append(dim_K_)
            if (K == 0) or (L % 2 == 0 and K == L // 2):
                dim_P = []
                for P in (1, -1):
                    representative_basis_P, trans, trans_refl, dim_P_ = bh.gen_representative_basis_pkblock(basis_Z2, L, K, P)
                    dim_P.append(dim_P_)
                print(dim_K[-1], sum(dim_P))
        print(dim_Z2[-1], sum(dim_K))
    print(dim, sum(dim_Z2))
...
def main3(L: int, M: int, Z2: int):
    sector_to_dimension = {}
    basis_Z2, dim_Z2 = bh.gen_basis_z2block(L, Z2, M)
    sector_Z2 = sector_to_dimension[('Z2', Z2, dim_Z2)] = {}
    for K in range(L):
        representative_basis_K, trans, dim_K = bh.gen_representative_basis_kblock(basis_Z2, L, K)
        sector_K = sector_Z2[('KZ2', K, Z2,  dim_K)] = {}
        if (K == 0) or (L % 2 == 0 and K == L // 2):
            for P in (1, -1):
                representative_basis_P, trans, trans_refl, dim_P = bh.gen_representative_basis_pkblock(basis_Z2, L, K, P)
                sector_P = sector_K[('PKZ2', P, K, Z2, dim_P)] = {}
    print(sector_to_dimension)
    def get_end_nodes(map: dict, end_nodes: list = None):
        if end_nodes is None:
            end_nodes = []
